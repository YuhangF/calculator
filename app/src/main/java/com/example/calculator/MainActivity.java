...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View v) {
        ...
        switch(v.getId()){
            case R.id.btn_del:
                if(str.length()>1){
                    str=str.substring(0,str.length()-1);  //substring
                }
                else{
                    str="";
                }
            ...
            case R.id.btn_9:
                ...
                break;
            case R.id.btn_p:
                if(str.equals("")||str.charAt(str.length()-1)=='+'||str.charAt(str.length()-1)=='-'||str.charAt(str.length()-1)=='×'||str.charAt(str.length()-1)=='÷'||str.charAt(str.length()-1)=='.'){
                    //如果当前字符串的最后一个字符是运算符或当前字符串为空时，报错！
            ...
            case R.id.btn_equ:
                ...
                if(str.charAt(str.length()-1)=='+'||str.charAt(str.length()-1)=='-'||str.charAt(str.length()-1)=='×'||str.charAt(str.length()-1)=='÷'){
                    Toast.makeText(MainActivity.this,"请完成公式!",Toast.LENGTH_SHORT).show();
                    text.setText(str);
                }
                else {
            ...
            case R.id.btn_squ2:
                ...
                if(str.charAt(0)=='-'){
                    ...
                    Toast.makeText(MainActivity.this,"标志符不能被开方",Toast.LENGTH_SHORT).show();
                    text.setText("0");
                }
                else{
            ...
            switch(ch){
                ...
                case '-':
                    while(!stack.isEmpty()&&!stack.peek().equals("(")) //如果栈不为空且栈顶元素不是"(",则出栈
                      postfix.append(stack.pop());   //且添加到后缀表达式字符串
                    stack.push(ch+""); //ch入栈
                    i++;
    ...
    public String getResult(){
        ...
        StringBuffer p=new StringBuffer(infix);   //通过转换StringBuffer调用其insert函数以达到修改运算公式的功能
        if(infix.charAt(0)=='-'){
            p.insert(0,0);
          //  Toast.makeText(MainActivity.this,"运算公式："+infix,Toast.LENGTH_SHORT).show();
        }
        for(int i=1;i<infix.length();i++){
            if(p.charAt(i)=='-'&&p.charAt(i-1)=='('){
                p.insert(i,0);
              //  Toast.makeText(MainActivity.this,"运算公式："+p,Toast.LENGTH_SHORT).show();
            }
        }
        infix=p.toString();
